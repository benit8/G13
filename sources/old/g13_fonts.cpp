...
G13::Font::Font(const std::string &name, unsigned int width)
: m_name(name)
, m_width(width)
{
}
...
void G13::FontChar::setCharacter(unsigned char *data, int width, unsigned flags)
{
...
static int GetFontCharacterCount(T(&)[size])
{
	return size;
}

template <class ARRAY_T, class FLAGST>
void G13::Font::install_font(ARRAY_T &data, FLAGST flags, int first)
{
...
void G13::Device::_init_fonts()
{
	m_current_font = FontPtr = new Font("8x8", 8);
	m_fonts[m_current_font->name()] = m_current_font;

	m_current_font->install_font(font8x8_basic, G13::FontChar::FF_ROTATE, 0);

	FontPtr fiveXeight = new G13::Font("5x8", 5);
	fiveXeight->install_font(font5x8, 0, 32);
	m_fonts[fiveXeight->name()] = fiveXeight;
}