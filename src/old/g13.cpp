...
void G13_Device::set_mode_leds(int leds)
{
	...
	if (r != 5) {
		G13::LOG(error, "Problem sending data");
		return;
	}
}
...
void G13_Device::set_key_color(int red, int green, int blue)
{
	...
	int error = libusb_control_transfer(handle, LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE, 9, 0x307, 0, usb_data, 5, 1000);
	if (error != 5) {
		G13::LOG(error, "Problem sending data");
		return;
	}
...
void G13_Manager::discover_g13s(libusb_device **devs, ssize_t count, vector<G13_Device*> &g13s)
{
	for (int i = 0; i < count; i++)
	{
		...
		if (r < 0) {
			G13::LOG(error, "Failed to get device descriptor");
			continue;
		}
		...
		if (desc.idVendor == G13_VENDOR_ID && desc.idProduct == G13_PRODUCT_ID) {
			...
			if (r != 0) {
				G13::LOG(error, "Error opening G13 device");
				continue;
			}

			if (libusb_kernel_driver_active(handle, 0) == 1) {
				if (libusb_detach_kernel_driver(handle, 0) == 0)
					G13::LOG(info, "Kernel driver detached");
			}

			r = libusb_claim_interface(handle, 0);
			if (r < 0) {
				G13::LOG(error, "Cannot claim interface");
				continue;
			}

			g13s.push_back(new G13::Device(*this, handle, g13s.size()));
		}
	}
}
...
{
	const char *dev_uinput_fname = NULL;
	if (access("/dev/input/uinput", F_OK) == 0) dev_uinput_fname = "/dev/input/uinput";
	else if (access("/dev/uinput", F_OK) == 0) dev_uinput_fname = "/dev/uinput";
	if (!dev_uinput_fname) {
		G13::LOG(error, "Could not find an uinput device");
		return -1;
	}
	if (access(dev_uinput_fname, W_OK) != 0) {
		G13::LOG(error, dev_uinput_fname << " doesn't grant write permissions");
		return -1;
	}

	int ufile = open(dev_uinput_fname, O_WRONLY | O_NDELAY);
	if (ufile <= 0) {
		G13::LOG(error, "Could not open uinput");
		return -1;
	}
	...
	if (retcode < 0) {
		G13::LOG(error, "Could not write to uinput device (" << retcode << ")");
		return -1;
	}
	retcode = ioctl(ufile, UI_DEV_CREATE);
	if (retcode) {
		G13::LOG(error, "Error creating uinput device for G13");
		return -1;
	}
	return ufile;
}
...
void G13_Device::register_context(libusb_context *_ctx)
{
	...
	m_input_pipe_name = _manager.make_pipe_name(this, true);
	m_input_pipe_fid = g13_create_fifo(m_input_pipe_name.c_str());
	m_output_pipe_name = _manager.make_pipe_name(this, false);
	m_output_pipe_fid = g13_create_fifo(m_output_pipe_name.c_str());

	if (m_input_pipe_fid == -1) {
		G13::LOG(error, "failed opening pipe");
	}
}
...
static std::string describe_libusb_error_code(int code)
{
	#define TEST_libusb_error(r, data, elem) \
		case BOOST_PP_CAT(LIBUSB_, elem): return BOOST_PP_STRINGIZE(elem);
...
void G13_Device::read_config_file(const std::string &filename) {
	...
	G13::LOG(info, "reading configuration from " << filename);
	while (s.good()) {
...
void G13_Device::read_commands() {
	...
	if (ret > 0) {
		...
		G13::LOG(trace, "read " << ret << " characters");
		...
		if (ret == 960) { // TODO probably image, for now, don't test, just assume image
			...
			BOOST_FOREACH(std::string const &cmd, lines) {
				...
				if (command_comment.size() > 0 && command_comment[0] != std::string("")) {
					G13::LOG(info,  "command: " << command_comment[0]);
...
G13::Device::Device(G13_Manager &manager, libusb_device_handle *handle, int _id)
: m_manager(manager)
, m_lcd(*this)
, m_stick(*this)
, m_handle(handle)
, m_id_within_manager(_id)
, m_uinput_fid(-1)
, m_ctx(0)
{
	m_current_profile = ProfilePtr(new G13::Profile(*this, "default"));
	m_profiles["default"] = m_current_profile;
...
void G13_Action_Keys::act(G13_Device &g13, bool is_down) {
	if (is_down) {
		for (int i = 0; i < _keys.size(); i++) {
			...
			G13::LOG(trace, "sending KEY DOWN " << _keys[i]);
		}
	} else {
		for (int i = _keys.size() - 1; i >= 0; i--) {
			g13.send_event(EV_KEY, _keys[i], is_down);
			G13::LOG(trace, "sending KEY UP " << _keys[i]);
		}
...
void G13_Device::_init_commands() {
	...
	G13_DEVICE_COMMAND(bind) {
		...
		try {
			...
			G13::LOG(trace,  "bind " << keyname << " [" << action << "]");
...
{
	G13::LOG(info, "set_string_config_value " << name << " = " << repr(value));
...
int G13_Manager::run()
{
	...
	if (ret < 0) {
		G13::LOG(error, "Initialization error: " << ret);
		return 1;
	}
	...
	if (cnt < 0) {
		G13::LOG(error, "Error while getting device list");
		return 1;
	}

	discover_g13s(devs, cnt, g13s);
	libusb_free_device_list(devs, 1);
	G13::LOG(info, "Found " << g13s.size() << " G13s");
	if (g13s.size() == 0) {
		return 1;
	}
